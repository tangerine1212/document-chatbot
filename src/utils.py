# code from: https://github.com/HITsz-TMG/TruthReader-document-assistant
...
def process_func(example, tokenizer, max_length=512):
    # Initialize lists for tokenized outputs
    ...
    qa_instruction = process_qa_prompt(
        ...
        tokenizer=tokenizer,
        max_context_len=5632
    )
    messages.append({"role": "user", "content": qa_instruction})

    instruction = tokenizer.apply_chat_template(messages, tokenize=False, add_special_tokens=False)
...
def process_qa_prompt(
    ...
    for i, item in enumerate(documents):
        ...
        page_id = item.get("page_id", f"doc_{i+1}") 
        document = f"""## 文档[{i+1}]\t{title}-{page_id}\n{text}"""
        documents_list.append(document)
